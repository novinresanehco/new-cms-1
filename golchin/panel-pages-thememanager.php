        ...
        case 'saveNewTheme':
            ...
            else
            {
                ...
                else
                {
                    ...
                    $menu             		= $d->query( "SELECT * FROM `ns_menu_group` WHERE `id`=1" );
					
					//save menu type
                    $menu = $d->query( "SELECT `style` FROM `ns_menu_group`" );
					$data = $d->fetch( $menu );
					$newTheme['menu'] = $data['style'];
					$newTheme['menu_items'] = array();
					//save menu items
					$menu = $d->query( "SELECT * FROM `ns_menu`" );
					while( $data = $d->fetch( $menu ) )
					{
						$newTheme['menu_items'][] = $data;
					}
					//save slider settings
					$newTheme['slider_thn_width'] 	= $config['slider_thn_width'];
					$newTheme['slider_thn_height'] 	= $config['slider_thn_height'];
					$newTheme['slider_group_style'] = $config['slider_group_style'];
					
					//save slider images
					$slider = $d->query( "SELECT * FROM `mp_slider`" );
					while( $data = $d->fetch( $slider ) )
					{
						$newTheme['slider_items'][] = $data;
					}
...
if ( isset( $_POST['posdata'] ) && is_array( $_POST['posdata'] ) )
{
	...
	{
		    saveconfig( 'theme_' . $config['theme'] . '_' . $_POST['theme'] . '_sidebars', json_encode( null ) );
	}
}
printpm( "ok", "success" );
