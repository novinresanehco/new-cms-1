...
$CMSq='';
if(isset($_GET['cat'])){$cat = intval($_GET['cat'],1); $CMSq="AND cat_id='".$cat."'";}
if(isset($_GET['id'])){$id = intval($_GET['id'],1); $CMSq="AND id='".$id."'";}
$dataq = $d->Query("SELECT * FROM data WHERE date <= '$ctimestamp' $CMSq ORDER BY id DESC LIMIT $news_limit");
		...
		$text  = str_replace('%pid%',$mdata['id'],$lang['protected']);
echo"
<item>
<title>$title</title>
