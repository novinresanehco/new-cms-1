...
define( 'head', true );
$pageTheme = 'list.htm';
define( 'page', 'sendpost' );
define( 'tabs', true );
$tabs = array( 'لیست مطالب', 'جستجو پیشرفته' );
$pagetitle = 'مدیریت مطالب کاربران';
include('header.php');
...
{
    $_POST['listids'] = explode( ',', $_POST['listids'] );
    ...
    if ( $_POST['method'] == 'draft' )
    {
        foreach ( $_POST['listids'] as $id )
        {
            $d->Query( "UPDATE `data` SET `show`='4' WHERE `id`='$id' AND `sendpost`='1' LIMIT 1" );
        }
    }
}
if ( isset( $_GET['deletepost'] ) )
{
    ...
    $d->Query( "DELETE FROM `data` WHERE id='$id' AND `sendpost`='1' " . $every . " LIMIT 1" );
}
if ( is_numeric( @$_GET['movedown'] ) )
{
    $wqu = ($permissions['editotherposts'] == 1) ? " " : " AND `author`='" . $info->userid . "' ";
    $qu = $d->Query( "SELECT `pos`,`id` FROM `data` WHERE `sendpost`='1' AND `pos`<'{$_GET['movedown']}' $wqu ORDER BY `pos` DESC LIMIT 1" );
    $qu = $d->fetch( $qu );
    $lowerid = $qu['id'];
    $qu = $qu['pos'];
    if ( !empty( $qu ) )
    {
        $d->Query( "UPDATE `data` SET `pos`='$qu' WHERE `pos`='{$_GET['movedown']}' AND `sendpost`='1' $wqu LIMIT 1" );
        $d->Query( "UPDATE `data` SET `pos`='{$_GET['movedown']}' WHERE `id`='$lowerid' AND `sendpost`='1' $wqu LIMIT 1" );
    }
    header( 'Location: sendpost.php' );
    exit( 'redirect' );
}
elseif ( is_numeric( @$_GET['moveup'] ) )
{
    $wqu = ($permissions['editotherposts'] == 1) ? " " : " AND `author`='" . $info->userid . "' ";
    $qu = $d->Query( "SELECT `pos`,`id` FROM `data` WHERE `pos`>'{$_GET['moveup']}' AND `sendpost`='1' $wqu ORDER BY `pos` ASC LIMIT 1" );
    $qu = $d->fetch( $qu );
    $uperid = $qu['id'];
    $qu = $qu['pos'];
    if ( !empty( $qu ) )
    {
        ...
        $d->Query( "UPDATE `data` SET `pos`='{$_GET['moveup']}' WHERE `id`='$uperid' AND `sendpost`='1' $wqu LIMIT 1" );
    }
    header( 'Location: sendpost.php' );
    exit( 'redirect' );
}
...
$q = $d->query( "SELECT * FROM `data`  WHERE (`author`='$info[u_id]' $q)  AND `sendpost`='1' $sq ORDER BY `pos` $type, `id` DESC LIMIT $From,$RPP" );
while ( $data = $d->fetch( $q ) )
{
...
{
    $q = $d->getrows( "SELECT `id` FROM `data` WHERE  `sendpost`='1' $sq ", true );
}
else
{
    $q = $d->getrows( "SELECT `id` as `num` FROM `data`  WHERE `author`='$info[u_id]'  AND `sendpost`='1' $sq ", true );
}
