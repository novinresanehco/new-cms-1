...
if ( file_exists( current_theme_dir . $themeSchema ) )
{
    if ( isset( $_GET['task'] ) )
    {
        ...
        {
            case 'pluginSetting':
                if ( empty( $_GET['plugin'] ) OR !ctype_alnum( $_GET['plugin'] ) )
                {
        ...
        else
        {
            ...
            foreach ( $themeSetting as $name => $data )
            {
                ...
                $tpl->block( 'Settingfields', array(
                    ...
                    'input' => generateField( $data['type'], "themesetting_$name", "themesetting_$name" . rand( 1000, 2000 ), isset( $data['class'] ) ? $data['class'] : '', isset( $data['value'] ) ? $data['value'] : ''  ),
                ) );
