...
function recent_output()
{
    ...
    $itpl       = new samaneh();
    $itpl->load( 'plugins/recent/block-theme.html' );
    ...
    $iq    = $d->Query( "SELECT * FROM `data` WHERE `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>$ctimestamp) AND (`show`!='4' || `show`='2') ORDER BY `id` DESC LIMIT  0,$count" );
...
$tpl->assign( 'recents', recent_output() );
