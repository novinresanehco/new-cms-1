...
$information = array(
    ...
    'data'        => array(
        ...
        'count' => array( 'name' => 'تعداد', 'value' => '10' ),
        'title' => array( 'name' => 'عنوان', 'value' => 'مطالب مشابه' ),
    )
);
$tpl->assign( 'first', '' );
if ( defined( 'methods' ) )
{
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='0' WHERE `name`='related' LIMIT 1" );
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='1' WHERE `name`='related' LIMIT 1" );
    ...
    {
        global $d;
        $q = $d->getrows( "SELECT `stat` FROM `plugins` WHERE `name`='related' LIMIT 1", true );
        if ( $q > 0 )
            print_msg( 'اين ماژول قبلا نصب شده است.', 'Info' );
        else
        {
            global $information;
            $q = $d->Query( "INSERT INTO `plugins` SET `name`='related',`title`='$information[name]',`stat`='0'" );
    ...
    function uninstallop()
    {
        ...
        {
            $q = $d->Query( "DELETE FROM `plugins` WHERE `name`='related' LIMIT 1" );
