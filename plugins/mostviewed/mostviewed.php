...
function mostviewed_output()
{
    ...
    $itpl = new samaneh();
    $itpl->load( 'plugins/mostviewed/block-theme.html' );
    ...
    $iq = $d->Query( "SELECT * FROM `data` WHERE `date` <= '$ctimestamp' AND (`expire`='0' OR `expire`='' OR `expire`>$ctimestamp) AND (`show`!='4' || `show`='2') ORDER BY `hits` DESC LIMIT  0,4" );
...
$tpl->assign( 'mostViewed', mostviewed_output() );