...
$information = array(
    ...
    'data'        => array( //لیست پارامترهای اختصاصی ماژول
        'title'  => array( 'name' => 'عنوان', 'value' => 'دیدگاه های اخیر' ),
        'count'  => array( 'name' => 'تعداد', 'value' => '10', 'type' => 'number' ),
        'length' => array( 'name' => 'طول نظر', 'value' => '50', 'type' => 'number' ),
...
if ( defined( 'methods' ) )
{
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='0' WHERE `name`='recentcomments' LIMIT 1" );
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='1' WHERE `name`='recentcomments' LIMIT 1" );
    ...
    {
        global $d;
        $q = $d->getrows( "SELECT `stat` FROM `plugins` WHERE `name`='recentcomments' LIMIT 1", true );
        if ( $q > 0 )
            print_msg( 'اين ماژول قبلا نصب شده است.', 'Info' );
        else
        {
            global $information;
            $q = $d->Query( "INSERT INTO `plugins` SET `name`='recentcomments',`title`='$information[name]',`stat`='0'" );
    ...
    function uninstallop()
    {
        ...
        {
            $q = $d->Query( "DELETE FROM `plugins` WHERE `name`='recentcomments' LIMIT 1" );
