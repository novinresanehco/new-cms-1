...
$information = array(
    ...
    'inactivate'  => true,
    'data'        => array(
        'title' => array( 'name' => 'عنوان', 'value' => 'محبوب ترین مطالب' ),
...
if ( defined( 'methods' ) )
{
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='0' WHERE `name`='hots' LIMIT 1" );
    ...
    {
        global $d;
        $d->Query( "UPDATE `plugins` SET `stat`='1' WHERE `name`='hots' LIMIT 1" );
    ...
    {
        global $d;
        $q = $d->getrows( "SELECT `stat` FROM `plugins` WHERE `name`='hots' LIMIT 1", true );
        if ( $q > 0 )
            print_msg( 'اين ماژول قبلا نصب شده است.', 'Info' );
        else
        {
            global $information;
            $q = $d->Query( "INSERT INTO `plugins` SET `name`='hots',`title`='$information[name]',`stat`='0'" );
    ...
    function uninstallop()
    {
        ...
        {
            $q = $d->Query( "DELETE FROM `plugins` WHERE `name`='hots' LIMIT 1" );
