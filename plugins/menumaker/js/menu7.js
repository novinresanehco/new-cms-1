...
jQuery(function(){
		...
		jQuery('ul.menu7 ul > li').addClass('noLava');
		jQuery('ul.menu7 > li').addClass('top-level');
		
		jQuery('ul.menu7 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menu7 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menu7 ul li.current-cat, ul.menu7 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menu7').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menu7').lavaLamp({ startItem: 0 });
		else jQuery('ul.menu7').lavaLamp();
