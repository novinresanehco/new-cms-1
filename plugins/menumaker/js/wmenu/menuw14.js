...
jQuery(function(){
		...
		jQuery('ul.menuw14 ul > li').addClass('noLava');
		jQuery('ul.menuw14 > li').addClass('top-level');
		
		jQuery('ul.menuw14 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menuw14 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menuw14 ul li.current-cat, ul.menuw14 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menuw14').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menuw14').lavaLamp({ startItem: 0 });
		else jQuery('ul.menuw14').lavaLamp();
