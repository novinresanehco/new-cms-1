...
jQuery(function(){
		...
		jQuery('ul.menuw45 ul > li').addClass('noLava');
		jQuery('ul.menuw45 > li').addClass('top-level');
		
		jQuery('ul.menuw45 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menuw45 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menuw45 ul li.current-cat, ul.menuw45 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menuw45').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menuw45').lavaLamp({ startItem: 0 });
		else jQuery('ul.menuw45').lavaLamp();
