...
jQuery(function(){
		...
		jQuery('ul.menuw69 ul > li').addClass('noLava');
		jQuery('ul.menuw69 > li').addClass('top-level');
		
		jQuery('ul.menuw69 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menuw69 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menuw69 ul li.current-cat, ul.menuw69 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menuw69').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menuw69').lavaLamp({ startItem: 0 });
		else jQuery('ul.menuw69').lavaLamp();
