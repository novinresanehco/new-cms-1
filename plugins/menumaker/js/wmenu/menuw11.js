...
jQuery(function(){
		...
		jQuery('ul.menuw11 ul > li').addClass('noLava');
		jQuery('ul.menuw11 > li').addClass('top-level');
		
		jQuery('ul.menuw11 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menuw11 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menuw11 ul li.current-cat, ul.menuw11 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menuw11').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menuw11').lavaLamp({ startItem: 0 });
		else jQuery('ul.menuw11').lavaLamp();
