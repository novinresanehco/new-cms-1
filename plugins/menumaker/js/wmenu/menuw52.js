...
jQuery(function(){
		...
		jQuery('ul.menuw52 ul > li').addClass('noLava');
		jQuery('ul.menuw52 > li').addClass('top-level');
		
		jQuery('ul.menuw52 > li > a.sf-with-ul').parent('li').addClass('sf-ul');
		...
		jQuery('ul.menuw52 li ul').append('<li class="bottom_bg noLava"></li>');
		
		var active_subpage = jQuery('ul.menuw52 ul li.current-cat, ul.menuw52 ul li.current_page_item').parents('li.top-level').prevAll().length;
		var isHome = 1; 
		
		if (active_subpage) jQuery('ul.menuw52').lavaLamp({ startItem: active_subpage });
		else if (isHome === 1) jQuery('ul.menuw52').lavaLamp({ startItem: 0 });
		else jQuery('ul.menuw52').lavaLamp();
