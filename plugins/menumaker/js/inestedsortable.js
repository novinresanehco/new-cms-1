/**
 * 
 * Nested Sortable Plugin for jQueryTemp/Interface.
 * 
 * Version 1.0.1
 *  
 *Change Log:
...
jQueryTemp.iNestedSortable = {
	...
	checkHover : function(e,o) {
		if(e.isNestedSortable){
			...
			jQueryTemp.iNestedSortable.scroll(e);
			return jQueryTemp.iNestedSortable.newCheckHover(e);
		} else {
			//The legacy hovering check performed by plain Sortables
			//I don't think "o" is even used, but it is in the function declaration
			return jQueryTemp.iNestedSortable.oldCheckHover(e,o);
		}
	},
	oldCheckHover : jQueryTemp.iSort.checkhover,
	newCheckHover : function (e) {
		...
		if(!e.nestedSortCfg.remeasured) {
			jQueryTemp.iSort.measure(e);
			e.nestedSortCfg.remeasured = true;
		}
		
		//finds item that is on top of the one being dragged,
		//and in a compatible nesting level
		var precItem = jQueryTemp.iNestedSortable.findPrecedingItem(e);
		
		var shouldNest = jQueryTemp.iNestedSortable.shouldNestItem(e, precItem);
		var touchingFirst = (!precItem) ? jQueryTemp.iNestedSortable.isTouchingFirstItem(e) : false;
		var quit = false;
		...
		if (precItem !== null) {
			...
			if (shouldNest) {
				jQueryTemp.iNestedSortable.nestItem(e, precItem);
			} else  {
				jQueryTemp.iNestedSortable.appendItem(e, precItem);
			}
		} else if (touchingFirst) {
			//no element on top, but touches the first item on the list
			jQueryTemp.iNestedSortable.insertOnTop(e);	
		}
	...
	scroll: function(e) {
		...
		var pointer = jQueryTemp.iDrag.dragged.dragCfg.currentPointer;
		var docDim = jQueryTemp.iUtil.getScroll();
	...
	check : function(dragged) {
		...
		jQueryTemp.iNestedSortable.newCheck(dragged);

		//The legacy release check performed by plain Sortables
		return jQueryTemp.iNestedSortable.oldCheck(dragged);
	},
	oldCheck : jQueryTemp.iSort.check,
	newCheck : function(dragged) {
		//removes nesting styling
		if (jQueryTemp.iNestedSortable.latestNestingClass &&
			jQueryTemp.iNestedSortable.currentNesting) 
		{
			jQueryTemp.iNestedSortable.currentNesting
				.removeClass(jQueryTemp.iNestedSortable.latestNestingClass);
			jQueryTemp.iNestedSortable.currentNesting = null;
			jQueryTemp.iNestedSortable.latestNestingClass = "";
		}
		
		if(jQueryTemp.iDrop.overzone.isNestedSortable) {
			jQueryTemp.iDrop.overzone.nestedSortCfg.remeasured = false;
		}
	...
	serialize: function(s) {
		if(jQueryTemp('#' + s).get(0).isNestedSortable){
			//A brand new NestedSortable serialization
			return jQueryTemp.iNestedSortable.newSerialize(s);
		} else {
			//The legacy serialization
			return jQueryTemp.iNestedSortable.oldSerialize(s);
		}
	},
	oldSerialize: jQueryTemp.iSort.serialize,
	newSerialize: function (s) {
		...
		var buildHierarchySer = function(context) {
			...
			thisChildren.each( function(i) {
				...
				retVal[i] = {id: serId};
				var newContext = jQueryTemp(this).children(e.nestedSortCfg.nestingTag + "." + e.nestedSortCfg.nestingTagClass.split(" ").join(".")).get(0);
	...
	findPrecedingItem : function (e) {
		var largestY = 0;
		var preceding = jQueryTemp.grep(e.dropCfg.el, function(i) {
			//needs to be on top of the one being dragged
			var isOnTop = (i.pos.y < jQueryTemp.iDrag.dragged.dragCfg.ny) && (i.pos.y > largestY);
			if(!isOnTop) {
				return false;
			}
			...
			if(e.nestedSortCfg.rightToLeft) {
				isSameLevel = (i.pos.x + i.pos.wb + e.nestedSortCfg.snapTolerance > jQueryTemp.iDrag.dragged.dragCfg.nx + jQueryTemp.iDrag.dragged.dragCfg.oC.wb);
			} else {
				isSameLevel = (i.pos.x - e.nestedSortCfg.snapTolerance < jQueryTemp.iDrag.dragged.dragCfg.nx);
			}
			if(!isSameLevel) {
				return false;
			}
			...
			var isBeingDragged = jQueryTemp.iNestedSortable.isBeingDragged(e, i);
			if(isBeingDragged) {
				return false;
			}
	...
	isTouchingFirstItem : function (e) {
		var lowestY;
		var firstItem = jQueryTemp.grep(e.dropCfg.el, function(i) {
		...
		if ( firstItem.length > 0 ) {
			...
			return firstItem.pos.y < jQueryTemp.iDrag.dragged.dragCfg.ny + jQueryTemp.iDrag.dragged.dragCfg.oC.hb &&
				firstItem.pos.y > jQueryTemp.iDrag.dragged.dragCfg.ny;
		} else {
			return false;
		}
	},
	...
	shouldNestItem : function(e, precedingItem ) {
		...
		if(e.nestedSortCfg.noNestingClass &&
			jQueryTemp(precedingItem).filter("." + e.nestedSortCfg.noNestingClass).get(0) === precedingItem)
			{return false;}
		...
		if(	e.nestedSortCfg.nestingLimit !== false) {
			...
			var nLevelPrec = jQueryTemp(precedingItem)
				.parents("." + e.sortCfg.accept.split(" ").join("."))
				.length;
			...
			jQueryTemp(e.nestedSortCfg.nestingTag + "." + e.nestedSortCfg.nestingTagClass.split(" ").join(".") + " > ." + e.sortCfg.accept.split(" ").join(".") + ":first-child", jQueryTemp.iDrag.dragged)
				.not("*["+e.nestedSortCfg.nestingTag + "." + e.nestedSortCfg.nestingTagClass.split(" ").join(".")+"]")
				.each( function(i) {
					var draggedIndex = 0;
					//finds out 
					var parentLength = jQueryTemp(this).parents("." + e.sortCfg.accept.split(" ").join(".")).each(
						function(i) {
							draggedIndex = i;
							return this == jQueryTemp.iDrag.dragged;
						}
						).length;
		...
		if (e.nestedSortCfg.rightToLeft) {
			return precedingItem.pos.x + precedingItem.pos.wb - (e.nestedSortCfg.nestingPxSpace - e.nestedSortCfg.snapTolerance) > jQueryTemp.iDrag.dragged.dragCfg.nx + jQueryTemp.iDrag.dragged.dragCfg.oC.wb;
		} else {
			return precedingItem.pos.x + (e.nestedSortCfg.nestingPxSpace - e.nestedSortCfg.snapTolerance) < jQueryTemp.iDrag.dragged.dragCfg.nx;
		} 
	},
	nestItem: function(e, parent) {
		//selects the nesting tag inside the parent
		var parentNesting = jQueryTemp(parent).children(e.nestedSortCfg.nestingTag + "." + e.nestedSortCfg.nestingTagClass.split(" ").join(".") );
		var helper = jQueryTemp.iSort.helper;
		...
		if ( !parentNesting.size()) {
			...
			parentNesting = jQueryTemp(parent).append(newUl).children(e.nestedSortCfg.nestingTag).css(e.nestedSortCfg.styleToAttach); 
		}
		
		//styles the nesting
		jQueryTemp.iNestedSortable.updateCurrentNestingClass(e, parentNesting );
		
		//does stuff before the helper is removed
		jQueryTemp.iNestedSortable.beforeHelperRemove(e);
		
		//puts the helper in the proper place.
		parentNesting.prepend(helper.get(0));
		
		//does stuff after the helper is inserted
		jQueryTemp.iNestedSortable.afterHelperInsert(e);
	},
	appendItem: function(e, itemBefore) {
		jQueryTemp.iNestedSortable.updateCurrentNestingClass(e, jQueryTemp(itemBefore).parent() );
		jQueryTemp.iNestedSortable.beforeHelperRemove(e);
		jQueryTemp(itemBefore).after(jQueryTemp.iSort.helper.get(0));
		jQueryTemp.iNestedSortable.afterHelperInsert(e);
	},
	insertOnTop: function (e) {
		jQueryTemp.iNestedSortable.updateCurrentNestingClass(e, e);
		jQueryTemp.iNestedSortable.beforeHelperRemove(e);
		jQueryTemp(e).prepend(jQueryTemp.iSort.helper.get(0));
		jQueryTemp.iNestedSortable.afterHelperInsert(e);
	},
	beforeHelperRemove : function (e) {
		//hides the nesting when it becomes empty
		var parent = jQueryTemp.iSort.helper.parent(e.nestedSortCfg.nestingTag + "." + e.nestedSortCfg.nestingTagClass.split(" ").join("."));
	...
	afterHelperInsert : function (e) {
		...
		var parent = jQueryTemp.iSort.helper.parent();
		if(parent.get(0) !== e) {
			parent.show();
		}
	...
	updateCurrentNestingClass : function(e, nestingElem) {
		...
		var nesting = jQueryTemp(nestingElem);
		
		if ((e.nestedSortCfg.currentNestingClass) && //makes sure a special class is desired
			(!jQueryTemp.iNestedSortable.currentNesting || nesting.get(0) != jQueryTemp.iNestedSortable.currentNesting.get(0)) ){ //avoids doing it on the same elem
			
			//removes from last nesting
			if(jQueryTemp.iNestedSortable.currentNesting) {
				jQueryTemp.iNestedSortable.currentNesting
					.removeClass(e.nestedSortCfg.currentNestingClass);
			}		
			
			if(nesting.get(0) != e) { //not root nesting
				jQueryTemp.iNestedSortable.currentNesting = nesting;
				
				//adds to this one
				nesting.addClass(e.nestedSortCfg.currentNestingClass);
				
				//this is need to remove the styling on "check"
				jQueryTemp.iNestedSortable.latestNestingClass = e.nestedSortCfg.currentNestingClass;
			} else {
		...
		return this.each(
			function () {
				if(this.isNestedSortable) {
					...
					jQueryTemp(this).SortableDestroy();
				}
			}
		);
	},
	build: function(conf) {
		if (conf.accept && jQueryTemp.iUtil && jQueryTemp.iDrag && jQueryTemp.iDrop && jQueryTemp.iSort) 
		{
			this.each(
				function() {
					...
					jQueryTemp(this.nestedSortCfg.nestingTag, this)
			...
			jQueryTemp.iSort.checkhover = jQueryTemp.iNestedSortable.checkHover;
			jQueryTemp.iSort.check = jQueryTemp.iNestedSortable.check;
			jQueryTemp.iSort.serialize = jQueryTemp.iNestedSortable.serialize;
		}
...
//Extends jQueryTemp to add the plugin.
jQueryTemp.fn.extend(
	{
		NestedSortable : jQueryTemp.iNestedSortable.build,
		NestedSortableDestroy: jQueryTemp.iNestedSortable.destroy
	}
);
...
jQueryTemp.iUtil.getScroll = function (e)
	{
		var t, l, w, h, iw, ih;
